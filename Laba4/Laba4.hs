--  Обчислення first1(A) для КВ-граматик
module Main where
...
instance Show GrammarError where
  show EmptyGrammar =
    "ПОМИЛКА: граматика не мiстить жодного правила."
...
parseSymbol "eps"   = Right Eps
parseSymbol ""      = Left "Порожнiй символ у правiй частинi правила."
parseSymbol s@(c:_)
  | c `elem` ['A'..'Z'] = Right (NonT s)
  | otherwise            = Right (Term s)
  ...
  in case ws of
       (nt : "->" : rhsParts) ->
         case null nt of
           ...
           False ->
             case null rhsParts of
               True  -> Left ("Права частина правила для [" ++ nt ++
                              "] порожня. Використайте eps для epsilon.")
               False ->
                 let results = map parseSymbol rhsParts
                     errors  = [e | Left e <- results]
                     syms    = [s | Right s <- results]
                 in case errors of
                      (e:_) -> Left e
                      []    -> Right (nt, syms)
       _ -> Left ("Неправильний формат: \"" ++ line ++
...
header :: IO ()
header = do
  putStrLn "     Обчислення first1(A) для КВ-граматик"
...
-- Тест 1: базовий
-- S -> a B,  B -> b | eps
-- Очiкується: first1(S)={a}, first1(B)={b,eps}
grammar1 :: Grammar
grammar1 =
  [ ("S", [Term "a", NonT "B"])
  , ("B", [Term "b"])
...
-- Тест 2: ланцюгове виведення
-- S -> A B,  A -> a | eps,  B -> b | c
-- Очiкується: first1(S)={a,b,c}, first1(A)={a,eps}, first1(B)={b,c}
grammar2 :: Grammar
grammar2 =
  [ ("S", [NonT "A", NonT "B"])
  , ("A", [Term "a"])
...
-- Тест 3: всi нетермiнали виводять eps
-- S -> A B,  A -> eps,  B -> eps
-- Очiкується: first1(S)={eps}, first1(A)={eps}, first1(B)={eps}
grammar3 :: Grammar
grammar3 =
  [ ("S", [NonT "A", NonT "B"])
  , ("A", [Eps])
...
-- Тест 4: лiворекурсивна граматика
-- S -> S a | b
-- Очiкується: first1(S)={b}
grammar4 :: Grammar
grammar4 =
  [ ("S", [NonT "S", Term "a"])
  , ("S", [Term "b"]) ]

-- Тест 5: довгий ланцюг нетермiналiв
-- A -> B,  B -> C,  C -> d | eps
-- Очiкується: first1(A)={d,eps}, first1(B)={d,eps}, first1(C)={d,eps}
grammar5 :: Grammar
grammar5 =
  [ ("A", [NonT "B"])
  , ("B", [NonT "C"])
...
-- Тест 6: арифметичнi вирази (класична LL(1)-граматика)
-- E->T E', E'->+ T E'|eps, T->F T', T'->* F T'|eps, F->(E)|id
-- Очiкується: first1(E)={(,id}, first1(E')={+,eps},
--             first1(T)={(,id}, first1(T')={*,eps}, first1(F)={(,id}
grammar6 :: Grammar
grammar6 =
  [ ("E",  [NonT "T",  NonT "E'"])
...
-- Тест 7: взаємна рекурсiя
-- A -> B a | c,  B -> A b | d | eps
-- Очiкується: first1(A)={a,c,d}, first1(B)={a,c,d,eps}
grammar7 :: Grammar
grammar7 =
  [ ("A", [NonT "B", Term "a"])
  , ("A", [Term "c"])
...
-- Тест 8 (помилка): порожня права частина
grammarBad1 :: Grammar
grammarBad1 = [("S", [Term "a"]), ("B", [])]

-- Тест 9 (попередження): дублiкат правила
grammarBad2 :: Grammar
grammarBad2 = [("S", [Term "a"]), ("S", [Term "b"]), ("S", [Term "a"])]

-- Тест 10 (помилка): порожня граматика
grammarBad3 :: Grammar
grammarBad3 = []

-- Тест 11 (помилка): термiнал з порожнiм рядком
grammarBad4 :: Grammar
grammarBad4 = [("S", [Term ""])]
...
runAllBuiltin = do
  ...
  runBuiltinTest "ПОМИЛКА: термiнал з пустим рядком" grammarBad4
  separator
...
interactiveMode = do
  ...
  putStrLn "  Нетермiнал : починається з великої лiтери  (S, A, E, T')"
  putStrLn "  Термiнал   : починається з малої або спецсимвол  (a, +, id)"
  putStrLn "  Epsilon     : слово eps"
  putStrLn "Порожнiй рядок = кiнець введення."
  putStrLn ""
  rules <- collectRules []
  case rules of
